...
This file contains the main chumpy class that is used to create new derivable objects.
...
class Term(object):
    ...
    def __init__(self, default=None, desc=None, dr=True):
        ...
        # Add a creation_counter, a la Django models, so we can preserve the order in
        # which parameters are defined in the job.
...
class Ch(object):
    ...
    def __new__(cls, *args, **kwargs):
        ...
        import types
        if len(args) > 0 and isinstance(args[0], types.LambdaType):
            cls = ChLambda
    ...
    def __setattr__(self, name, value, itr=None):
        ...
        # TODO Raffi changed there
        if isinstance(self, Ch):
            if name == 'x':
    ...
    def show_tree(self, cachelim=np.inf):
        """Creates a visualization of the full expression tree, in Graphviz format.
